...
** Last update Sun Dec 21 18:32:33 2014 ugo belfiore
*/
...
void	aff_wolf_color(t_rayc *r, t_data *d)
{
  ...
  while (i++ < r->drawStart)
    {
      aff_pix_img(d, r->x, i);
      d->colo = my_putnbr_base_wolf(d->colo, 1, d->sd.poney);
    }
  ...
  while (r->drawEnd++ < r->tmp_ymax)
    {
      ...
      d->colo = my_putnbr_base_wolf(d->colo, 2, d->sd.poney);
    }
}

/*
** fonction qui affiche le sol en "miroir" par rapport au murs.
*/
...
void	aff_wolf_key_move(t_data *d, int keycode)
{
  ...
  if (d->ia.pop == 30)
    {
      if (my_getnbr(d->tab[d->ia.popX + d->x_file * d->ia.popY]) == 2)
	{
	  d->ia.stop = 2;
	  aff_win_lose(d);
	}
      else
	d->tab[d->ia.popX + d->x_file * d->ia.popY] = "50";
      d->ia.pop++;
    }
}
